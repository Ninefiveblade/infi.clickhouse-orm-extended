...
from .engines import (Merge, Distributed,
                      DictionaryLifetime, SourceClickhouse,
                      SourcePostgreSQL, FlatDictionaryLayout,
...
class ModelBase(type):
    ...
    def create_ad_hoc_field(cls, db_type):
        import infi.clickhouse_orm_extended.fields as orm_fields
        # Enums
...
class Dictionary(metaclass=ModelBase):
    ...
    def create_dictionary_sql(cls, db):
        ...
        for c in cls._constraints.values():
            items.append('    %s' % c.create_dictionary_sql())
        # Indexes
        for i in cls._indexes.values():
            items.append('    %s' % i.create_dictionary_sql())
        parts.append(',\n'.join(items))
        # Engine
        parts.append(')')
        parts.append('PRIMARY KEY %s' % comma_join(cls.primary_key, stringify=True))
        parts.append(cls.source.create_sql())
        parts.append(cls.lifetime.create_sql())
        parts.append(cls.layout.create_sql())
        return '\n'.join(parts)

    @classmethod
    def drop_dictionary_sql(cls, db):
        '''
